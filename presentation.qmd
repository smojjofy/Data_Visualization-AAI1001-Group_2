...
- Weaknesses:
  - No on-chart legend â€” colors cannot be decoded without external report tables
...
- Checks are coded to fail fast: missing values, percentages out of bounds, duplicate rows, and composition sums.
- Each country-dimension pair is required to sum to `100 +/- 0.01` for compositional integrity.
- Render commands are fixed and repeatable from the project root: `quarto preview presentation.qmd` and `quarto render presentation.qmd`.
